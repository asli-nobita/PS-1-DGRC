...
export default function SplashScreen({ onFinish }) {
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            onFinish(); // Call the callback provided by the parent (App.js)
        }, 3000); // Display splash for 3 seconds as per common practice

        return () => clearTimeout(timer); // Clean up the timer
    }, [onFinish]); // Dependency array to re-run effect if onFinish changes
